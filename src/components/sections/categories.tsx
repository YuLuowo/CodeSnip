...
export default function Categories({ categories }: CategoryProps) {
    ...
    return (
        <section className="flex flex-col gap-4 w-full max-w-4xl px-4 md:px-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {categories.map((category, index) => (
                    <Link key={category._id} href={`/search?page=1&tag=${category._id}`} className="flex items-center p-4 gap-4 border border-muted rounded-lg shadow-sm hover:shadow-md hover:bg-muted/40 transition-shadow duration-300 cursor-pointer">
